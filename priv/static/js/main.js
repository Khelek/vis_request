...
function animationSprite(sprite) {
	...
	var scale = {x: 6, y: 6, z: 1};
	var tweenVisible = new TWEEN.Tween(scaleCurrent).to(scale, 1500);
	tweenVisible.onUpdate(function() {
		sprite.scale.x = scaleCurrent.x;
		sprite.scale.y = scaleCurrent.y;
		sprite.scale.z = scaleCurrent.z;
	});
	var paramColor = {r : sprite.material.color.r, g: sprite.material.color.g, b: sprite.material.color.b};
	var redColor = {r: 3, g: 0, b:0};
	var blueColor = {r: 1, g: 2, b:3};
	var tweenToRed = new TWEEN.Tween(paramColor).to(redColor, 500);
	var tweenToBlue =  new TWEEN.Tween(paramColor).to(blueColor, 500);
	tweenToRed.onUpdate(function(){
            sprite.material.color.r = paramColor.r;
            sprite.material.color.g = paramColor.g;
            sprite.material.color.b = paramColor.b;
        });
    	tweenToBlue.onUpdate(function() {
        sprite.material.color.r = paramColor.r;
        sprite.material.color.g = paramColor.g;
        sprite.material.color.b = paramColor.b;
	});
	tweenToRed.chain(tweenToBlue);
	tweenToRed.start();
	tweenVisible.start();
}
